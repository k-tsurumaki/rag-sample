...
"""
    ...
    def _display_enhanced_result(self, result: Dict):
        ...
        if "extraction_summary" in result:
            ...
            print("\n【抽出サマリー】")
            print(f"  • 総抽出数: {summary['total_extractions']}")

            if summary["extraction_types"]:
                print("  • 抽出タイプ別統計:")
                for ext_type, count in summary["extraction_types"].items():
                    print(f"    - {ext_type}: {count}件")

            if summary["numerical_data"]:
                print("  • 数値データ:")
                for data in summary["numerical_data"][:3]:  # 上位3つ
                    print(f"    - {data['value']} ({data.get('context', {})})")

            if summary["technical_terms"]:
                print("  • 技術用語:")
                for term in summary["technical_terms"][:3]:  # 上位3つ
                    print(f"    - {term['term']}")
